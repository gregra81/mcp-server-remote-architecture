...
async function testEnhancedManager() {
  ...
  const remoteConfig: RemoteApiConfig = {
    enabled: true,
  ...
  console.log('   🌐 Remote tools:', remoteOnlyTools.map(t => t.name).join(', '));
  ...
  if (remoteOnlyTools.length > 0) {
    try {
      ...
      const remoteResult = await remoteManager.callTool(remoteOnlyTools[0].name, {
        amount: 100,
        rate: 0.08
      });
      console.log('   ✅ Remote tool result:', remoteResult.result);
    } catch (error) {
      console.log('   ⚠️  Remote tool failed (expected if server not running):', 
  ...
  try {
    ...
    console.log('   ✅ Validation correctly caught missing parameter:', 
                error instanceof Error ? error.message : error);
  }
  ...
  try {
    ...
    console.log('   ✅ Validation correctly caught missing city parameter:', 
                error instanceof Error ? error.message : error);
  }
