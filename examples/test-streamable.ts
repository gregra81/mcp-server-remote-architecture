...
async function testMCPStreamableServer(): Promise<void> {
  ...
  try {
    ...
    const toolsResponse: AxiosResponse = await axios.get(
      `${SERVER_URL}/mcp/tools`
    );
    ...
    const httpPostResult: AxiosResponse = await axios.post(
      `${SERVER_URL}/mcp/call-tool`,
      {
        tool: 'http_post',
        parameters: {
    ...
    const weatherResult: AxiosResponse = await axios.post(
      `${SERVER_URL}/mcp/call-tool`,
      {
        tool: 'get_weather',
        parameters: {
    ...
    const streamingResult: AxiosResponse = await axios.post(
      `${SERVER_URL}/mcp/call-tool-stream`,
      {
        tool: 'get_weather',
        parameters: {
          city: 'London',
      ...
      streamingResult.data.on('end', () => {
        ...
        lines.forEach(line => {
          try {
            ...
            if (data.type === 'result') {
              console.log(
                `   🌡️  Result: ${data.result.result.city} - ${data.result.result.temperature}°C`
              );
            }
          } catch (e) {
            // Ignore parse errors
  ...
  try {
    ...
    lines.forEach(line => {
      try {
        const data = JSON.parse(line);
        console.log(
          `   📨 Received: ${data.type} - ${data.message || data.clientId || 'capabilities'}`
        );
      } catch (e) {
        // Ignore parse errors
      }
    ...
    const chunkedResponse: AxiosResponse = await axios.get(
      `${SERVER_URL}/mcp/stream`,
      {
        responseType: 'stream',
        timeout: 5000,
      }
