...
app.get('/mcp/stream', (req: Request, res: Response): void => {
  ...
  res.write(
    Buffer.from(
      initialMessage.length.toString(16) + '\r\n' + initialMessage + '\r\n',
      'utf8'
    )
  );
  ...
  res.write(
    Buffer.from(
      capabilitiesMessage.length.toString(16) +
        '\r\n' +
        capabilitiesMessage +
        '\r\n',
      'utf8'
  ...
  const keepAlive = setInterval(() => {
    ...
    res.write(
      Buffer.from(
        pingMessage.length.toString(16) + '\r\n' + pingMessage + '\r\n',
        'utf8'
      )
    );
  }, 30000);
...
app.listen(port, (): void => {
  ...
  console.log(
    `📡 Simple streaming endpoint: http://localhost:${port}/mcp/stream-simple`
  );
  console.log(`🔧 Tool call endpoint: http://localhost:${port}/mcp/call-tool`);
  console.log(
    `⚡ Streaming tool call endpoint: http://localhost:${port}/mcp/call-tool-stream`
  );
